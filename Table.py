    ...
    def display(self):
        ...
        for i in enumerate(self.table):
            for j in enumerate(i[1]):
                if i[0] == 0:
                    print("| "+(Table.color_map[self.header_color] + str(j[1]) + Table.color_map["reset"])+" "*(
                        self.max_cell_size_by_column[j[0]] - len(str(j[1])) + 1), end="")
                else:
                    print("| "+(Table.color_map[self.body_color] + str(j[1]) + Table.color_map["reset"])+" "*(
                        self.max_cell_size_by_column[j[0]] - len(str(j[1])) + 1), end="")
